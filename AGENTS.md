...
**Current state (as of session `20260801-154329`, 100/100 delibere questions): this requirement is unmet for every single delibera/determina in the KB**, and the gap is in ingestion data, not prompt wording. Verified directly against `kb.db`: all 669 `documents` rows with `section='delibere'` have `source='manual'` and zero contain a `url` key in `metadata`. `CitedSource.source_ref` ([`backend/src/rag_engine/types.rs`](./backend/src/rag_engine/types.rs)) is documented as able to hold "URL, filename, or upload label" — the type already supports a URL — but every manual upload of a delibera/determina populated it with a bare filename instead. No amount of system-prompt instruction (`.project/training/*.md`) can make the bot cite a link that was never captured at ingestion time; fixing this requires either (a) recording the source URL at manual-upload time and propagating it through `CitedSource`, or (b) deriving it from a known public naming convention for the comune's document portal if one exists. Until one of these ships, expect every delibera/determina answer to fail this rule, and expect test-session reports to say so explicitly rather than silently pass citation on a filename.

Once real URLs are available, prefer having the **application layer append the link(s) programmatically** from `CitedSource` after generation, rather than relying on the generation model to reproduce a URL string verbatim in free text — every test session run against this KB so far has found the 1.5B generation model unreliable at reproducing exact strings (numbers, dates, even the literal configured fallback text; see `.project/training/fallback-text-adr-0012.md`), so a model-typed URL is a likely source of broken/hallucinated links.
