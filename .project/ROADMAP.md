...
- [x] **0009** — `/admin/api/upload` — per-section manual document upload
  ...
  - Closed: Plan [0009](../.project/0009-admin-api-upload-plan.md), ADR [0008](../.adr/0008-preview-confirm-upload-workflow.md).

- [x] **0010** — `/admin/api/ingest/config` — read/write ingest configuration
...
- [x] **0013** — `/admin/api/training/sessions/:id/messages` — ask/answer with recording
...
- [x] **0016** — admin-ui Ingest configuration section
  ...
  - Closed: Plan [0016](../.project/0016-admin-ui-ingest-configuration-section-plan.md). No ADR — thin UI CRUD wrapper over the existing feature 0009-0011 admin endpoints, no architectural decision.

- [x] **0017** — admin-ui Bot imprinting section
  ...
  - Closed: Plan [0017](../.project/0017-admin-ui-bot-imprinting-section-plan.md). No ADR — thin UI CRUD wrapper over the existing feature 0008 persona admin endpoints, no architectural decision.

- [x] **0018** — admin-ui Training section with point-in-answer feedback
  ...
  - Closed: Plan [0018](../.project/0018-admin-ui-training-section-with-point-in-answer-feedback-plan.md). No ADR — thin UI wrapper over the existing feature 0012-0014 training admin endpoints; the sentence-segmentation span-selection design is a UI implementation detail documented in the plan's Non-Goals, not a system-wide architectural decision.

- [x] **0019** — admin-ui accessibility + keyboard audit
  ...
  - Closed: Plan [0019](../.project/0019-admin-ui-accessibility-keyboard-audit-plan.md). No ADR — the fixes (live-region roles on dynamic callouts, a keyboard-reachable cron-help hint, and three 44px touch-target corrections) are bug fixes within the existing DSI component system, not a new architectural decision.
...
- [x] **0021** — frontend chat widget with citation rendering
  ...
  - Closed: Plan [0021](../.project/0021-frontend-chat-widget-with-citation-rendering-plan.md). No ADR — a thin, DSI-conformant UI component consuming the existing `/chat` DTO exactly as built in feature 0003; no new architectural decision (the necessary `/chat` nginx/vite proxy fix and `DsInput` placeholder extension are implementation-level corrections, not structural choices).

- [x] **0022** — frontend honest-unknown + error state UI
...
- [x] **0025** — End-to-end BDD against live containers
  ...
  - Closed: Plan [0025](../.project/0025-end-to-end-bdd-against-live-containers-plan.md). No ADR — a new test binary and Makefile target, not a structural architectural decision. Live verification during review surfaced a genuine, reproducible production observation (the default `min_score=0.35` retrieval threshold lets an unrelated document match a semantically-distant query with the real embedding model) — out of scope for this plan (no `rag_engine` changes), flagged as a candidate follow-up.

- [x] **0026** — Production hardening: non-root containers, resource limits, image scanning
...
## Milestone 6 — Content Population & Operational Tooling

Populating the live KB with real content (bot persona, `storia`/`news`/`delibere` sections, a new `Giunta` section) and adding operator convenience tooling for snapshotting/restoring `kb.db` state. Distinct in nature from Milestones 0-5: this is content curation and operational tooling on top of an already-shippable system (per `TEST-INGESTION-0001.md`'s live findings — `halleyweb.com` disallows scraping entirely, so `news`/`delibere`/`Giunta` sourcing requires manual per-document curation + upload, not a bulk scraper run), not new system architecture. When picked up via `/create-plan`, the planner should judge whether the content-population portion is better run as a dedicated operational plan under `.project/` (in the style of `TEST-INGESTION-0001.md`, executed incrementally via `/test-session`) versus a conventional Feature Plan — the Makefile tooling portion (`eject-data`/`use-data`) is the one piece that is a normal code feature end-to-end.

- [x] **0028** — Real content population (Gaspare persona, storia/news/delibere/Giunta ingestion) + kb.db snapshot Makefile targets
  - Description: Populate the live KB with real, curated content, and add operator data-snapshot tooling:
    - **Bot imprinting**: (re)populate the persona with name "Gaspare" and identity content grounded in Gaspare Spontini's real biography and his connection to Maiolati Spontini (see `TEST-INGESTION-0001.md` §4 for the verified anchor facts already gathered — born 14 Nov 1774, died 24 Jan 1851, name added to the comune in 1939).
    - **Populate three sections with the best possible real ingest**: `storia` (from the official comune page + Wikipedia + other reliable sites — the official page and any `halleyweb.com` page require the manual-upload workaround per the known robots.txt block, not a scrape source), `news` (last 6 months: **February 2026 through today, 2026-07-26** — real curated articles, not the bare teaser list), and `delibere e determine` (last 4 months: **April-May-June-July 2026** — real curated acts, individually sourced).
    - **New `Giunta` section**: sourced from `https://www.halleyweb.com/c042023/zf/index.php/organi-politico-amministrativo/index/index/categoria/78` (same `halleyweb.com` domain already confirmed to disallow scraping entirely — manual upload required).
    - **Admin credential stability**: the admin-ui/operator password must stop being rotated session-to-session. Keep it recorded in `secrets.txt` (repo root, gitignored, never committed) and treat it as long-lived — update the file in place if a rotation is ever genuinely required, but don't rotate as a side effect of routine test/ingest sessions.
    - **Makefile**: add `eject-data` (dump the live `kb.db`/stateful volume data to `.data/data-<yyyy-MM-dd>.bin`, a directory kept out of version control) and `use-data <data-file>` (restore a previously-ejected data file for use), so a known-good populated KB state can be snapshotted and restored on demand instead of re-running ingestion from scratch.
  - Closed: Plan [0028](../.project/0028-real-content-population-gaspare-persona-storia-news-delibere-giunta-ingestion-kb-db-snapshot-makefile-targets-plan.md). No ADR — per this milestone's own framing, content curation and operator snapshot tooling on top of an already-shippable system, not a new structural decision; the persona/content changes are pure data via the existing, already-reviewed admin API (features 0008/0009), and `eject-data`/`use-data` are thin operational Makefile wrappers, consistent with the no-ADR precedent for CRUD/tooling wrappers (features 0007, 0010, 0011).
