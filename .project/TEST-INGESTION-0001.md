...
- [ ] **3.3** Real quantitative verification (there's no dedicated admin endpoint to "count documents" — known gap, Appendix C). Method already verified in this very working session:
  ```bash
  ...
  ```
  Compare the count against the log in §5.3 (number of configured sources × expected chunks per document).
- [ ] **3.4** Minimal functional verification (smoke test) **before** launching the 1000 questions: 1 question per section with a known answer (e.g. "In che anno è morto Gaspare Spontini?" → must answer 1851 **and** cite the storia source). If even this smoke test fails, don't proceed to scale up: diagnose first (were chunk embeddings actually generated? is `min_score` too high for the chunk length? is the persona active?).
