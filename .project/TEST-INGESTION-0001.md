...
- Measured average latency (Wave 0, 20 real questions, 2026-07-26): **95.45 s**
- Measured p95 latency (Wave 0): **141.02 s** (nearest-rank method on the 20 sorted real latencies, range 6.99s–161.96s)
- Proposed target for the final gate: average ≤ **130 s** (~1.35× the measured baseline, leaving headroom for slower multi-fact questions without masking real regressions), p95 ≤ **180 s** (~1.3× the measured baseline). Both numbers are provisional — revisit once Wave 1/2's larger sample gives a more stable distribution; this is a 20-question sample, not a robust percentile estimate.
