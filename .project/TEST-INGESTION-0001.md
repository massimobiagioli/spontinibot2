...
- **`/chat` flow**: `POST /chat {"question": "..."}` → `RagEngine` (`backend/src/rag_engine`) → embeds the question (`llama-embed`, `nomic-embed-text` model, 768 dims) → cosine-similarity retrieval against `kb.db` (libSQL) — **as of 2026-07-26, `top_k=2`, `min_score=0.6`, both env-overridden in `docker-compose.yml` (code defaults remain `top_k=5`/`min_score=0.35`, see `backend/src/config.rs`) — changed live during this campaign, see Appendix C and [ADR 0013](../.adr/0013-generation-model-1-5b-and-reduced-rag-top-k-for-latency.md)** → **if zero chunks are retrieved above threshold → honest fallback, the generation model is never even called** (Constitution §5, guaranteed at the code level, not just by the prompt) → otherwise a 3-part prompt with separate sections (persona / retrieved context / question) → `llama-generate` (**Qwen2.5-1.5B-Instruct, Q4_K_M, as of 2026-07-26 — [ADR 0013](../.adr/0013-generation-model-1-5b-and-reduced-rag-top-k-for-latency.md), supersedes ADR 0001's original Qwen2.5-3B-Instruct choice**) → answer.
...
| 4 | storia | Organi politico-amministrativi (current sindaco/assessori/consiglio roster) | halleyweb.com/.../organi-politico-amministrativo/..., uploaded as `organi-politico-amministrativo.md` (real text fetched 2026-07-26, robots.txt disallows `/` for this host entirely — same manual-upload workaround) | n/a (live roster page, no single "document date"; fetched/ingested 2026-07-26) | manual upload (feature 0009, preview confirmed) — **ingested 2026-07-26 mid-Wave-0**, 1 chunk, document_id 30. Added to fix a confirmed hallucination found live during Wave 0 (Q19, §8) — the ingested Wikipedia text was stale on who's mayor; this source gives the real current roster. | ☑ |
...
**Exercised for real 2026-07-26 against Wave 0** (§10.3's 20-question latency baseline — the smallest, plan-mandated first wave, "before everything else"). 5.1-5.4 are generic mechanics applied per-wave (5.1 itself says "one training session per test wave"); this session applied them to Wave 0 specifically. The same mechanics apply unchanged to Wave 1/2 in future sessions.

- [x] **5.1** Create one training session per test wave (e.g. one session per 100 questions, or one per category): `POST /admin/api/training/sessions {"title": "Wave 1 — Category A/B/C", "created_by": "test-ingestion"}`. Ran 2026-07-26: `POST /admin/api/training/sessions {"title": "Wave 0 — latency baseline", "created_by": "test-ingestion"}` → `id:1`, HTTP 201.
- [x] **5.2** For every question: call `POST /admin/api/training/sessions/:id/messages {"question": "..."}`, **timing the response on the client side** (the `training_message`'s `created_at` field does not record latency — it must be timed externally, e.g. `curl -w "%{time_total}"` or a scripted wrapper). Ran 2026-07-26: all 20 Wave 0 questions (5 each from Categories A/B/C/F, verbatim from §7.3/§7.7) sent to `POST /admin/api/training/sessions/1/messages`, each timed with `curl -w "%{time_total}"`. One real snag: the session cookie's 30-minute TTL (`SESSION_TTL_SECS`, `backend/src/config.rs`) expired mid-run after question 9 (cumulative wait time crossed the 30-minute mark since login) — questions 10-20 initially failed with `401 invalid or missing session cookie`. Re-logged in and re-ran questions 10-20 for real (re-authenticating before each call this time); no invented data was substituted for the failed attempts.
- [x] **5.3** Log every exchange in a structured external log (CSV or similar), **in addition to** the training session — the training session is the canonical product-side record (reuses `RagEngine`, persists `sources`/`fell_back`), but aggregate scoring needs a log with extra columns:
  ...
  Filled in for real for all 20 questions — see §10.3 for aggregate latency and the summary below for the scoring headline. Full per-question detail (including the required Domanda/Risposta attesa/Risposta bot/Tempo di risposta/Feedback breakdown) is in `.project/test-reports/TEST-INGESTION-0001-rep.md`.
- [x] **5.4** Save the log under `.project/test-ingestion-results/wave-<N>-<date>.csv` (create the folder; don't version any sensitive data that surfaces during testing). Saved to `.project/test-ingestion-results/wave-0-2026-07-26.csv`, 20 real rows, no sensitive data (no operator credentials, no session tokens — only question/answer/citation/score data).

**Wave 0 headline (real, computed from the 20 rows above)**: average score 71.4/100 across Categories A/B/C/F. Two systemic problems found, both documented in Appendix C:
1. **Every single one of the 20 questions retrieved exactly 5 sources and `fell_back:false`** — including all 5 Category F (should-refuse) questions and 3 of 5 Category A identity questions that shouldn't need any document citation at all. The honest-fallback mechanism (§1: "zero chunks above threshold → generation model never even called") never triggered once in this sample. This is the `RAG_MIN_SCORE` permissiveness risk already flagged in Appendix C, now confirmed with hard data.
2. **One confirmed, important hallucination** (Q19, "Il prossimo sindaco chi sarà?"): answered a future-prediction question (now explicitly forbidden by [ADR 0012](../.adr/0012-categorical-refusal-rules-and-standard-fallback-text.md)) with an outdated name from the ingested Wikipedia text instead of refusing. Verified wrong via a real external source the user provided; the real current mayor is Sebastiano Mazzarini, not the name the bot gave. A corrective document (`organi-politico-amministrativo.md`) was ingested mid-session (§5.3 log) and a post-hoc, present-tense verification question now answers correctly — but Q19's row in the CSV/report reflects exactly what was observed during Wave 0, not retroactively corrected.
...
| Generation model | [ADR 0013](../.adr/0013-generation-model-1-5b-and-reduced-rag-top-k-for-latency.md) (supersedes ADR 0001) | **Changed 2026-07-26**: Qwen2.5-3B → Qwen2.5-1.5B-Instruct, after Wave 0's real ~95s average latency prompted a live benchmarking investigation (real hardware, real KB content) — see Appendix C. Real, measured ~2-3x latency improvement, quality held up in testing. Still not an ad-hoc change: went through a proper superseding ADR with real comparative data, not a guess. |
...
- Measured average latency (Wave 0, 20 real questions, 2026-07-26, **Qwen2.5-3B + top_k=5 — since replaced, see below**): **95.45 s**
- Measured p95 latency (Wave 0, same old config): **141.02 s** (nearest-rank method on the 20 sorted real latencies, range 6.99s–161.96s)
- ~~Proposed target for the final gate: average ≤ 130 s, p95 ≤ 180 s~~ — **superseded 2026-07-26.** The project owner asked for a <5s target after seeing Wave 0's numbers. Real live benchmarking (Appendix C, [ADR 0013](../.adr/0013-generation-model-1-5b-and-reduced-rag-top-k-for-latency.md)) found <5s is not achievable on this hardware at any tested model size without an unacceptable quality regression — the hardware floor for even the smallest usable model (1.5B) with near-zero context is ~7.5s. The generation model was changed to Qwen2.5-1.5B and `RAG_TOP_K` lowered to 2, giving a real, measured ~30-45s per question (down from ~95s) — verified live against the running stack, not just the isolated benchmark. **The 95.45s/141.02s numbers above are now stale**: they describe the old (3B, top_k=5) configuration, not the system as it currently runs. A fresh Wave 0-equivalent latency re-measurement against the new config is real follow-up work for the next session, not yet done — do not silently treat ~30-45s as validated against a full 20-question sample the way the original numbers were.
...
- **`halleyweb.com` (the domain hosting the news list, determine search, and the political-administration roster) disallows `/` in `robots.txt` for the generic user-agent** — every real source on that domain needs the manual-upload workaround, not scraping. Confirmed 2026-07-26 while ingesting `organi-politico-amministrativo.md` (§5.3 row 4). Plan ahead for this when doing the real `news`/`delibere` curation (§5.2 point 2) — every one of those sources will need manual upload too, not `scrape source` config, since they're also on `halleyweb.com`.
- **Retrieval never returned zero chunks in Wave 0 — the honest-fallback path never actually triggered, confirmed with real data** (found 2026-07-26): all 20 of Wave 0's real questions (§8), including all 5 deliberately out-of-KB Category F questions and several Category A identity questions needing no citation at all, retrieved exactly `top_k=5` sources and got `fell_back:false`. This is the `RAG_MIN_SCORE` permissiveness risk already listed above, now confirmed empirically rather than theoretically: with a 30-document KB, `RAG_MIN_SCORE=0.35` appears to accept essentially anything. Directly caused Category F's Wave 0 score to cap at 50 on 4 of 5 questions (honest text, wrong structural fallback) and contributed to Q19's confirmed hallucination (irrelevant-but-accepted chunks gave the model something to (mis)use instead of nothing). **First lever to try, per §10.2**: raise `RAG_MIN_SCORE` and re-run the Category A/F subset (not all 1000) to check whether the fallback path starts triggering correctly, before touching anything else.

**Tried for real 2026-07-26 — inconclusive, see the dedicated entry below ("`RAG_MIN_SCORE` cannot be cleanly tuned...") for the full data.** Short version: no single threshold value cleanly separates relevant from irrelevant retrieved content for this KB/embedding-model combination. Left at `RAG_MIN_SCORE=0.6` as a modest, honest improvement over 0.35 — not a fix.

**Update 2026-07-26 (Wave 1 sub-batch, 29 real questions, full detail and per-question table in `.project/test-reports/TEST-INGESTION-0001-rep.md`)**: the F-question structural failure (`fell_back:false` with irrelevant citations instead of `fell_back:true`/zero citations) reproduced on all 6 new Category F questions tested — unchanged since Wave 0, confirming this is still open, not a regression.
- **One confirmed, important hallucination on a future-prediction question** (found 2026-07-26, Wave 0 Q19, "Il prossimo sindaco chi sarà?"): the bot answered with a name from the ingested Wikipedia text (a former mayor, deposed 23 October 2025 per that same source's own citations) as if it were a valid answer to a question about the *future* — instead of refusing. Verified genuinely wrong via a real external source the user supplied (`halleyweb.com/.../organi-politico-amministrativo/...`): the real current mayor as of 2026-07-26 is Sebastiano Mazzarini. This is exactly the failure mode [ADR 0012](../.adr/0012-categorical-refusal-rules-and-standard-fallback-text.md)'s "no predictions" rule now explicitly forbids. A corrective document was ingested (§5.3 row 4) and a post-hoc present-tense question now answers correctly, but the underlying behavior gap — nothing currently stops the model from treating "next X" as "most recent X mentioned in a retrieved chunk" — is a prompt/retrieval problem, not fixed by adding one document. Needs real attention in Wave 1 scoring, not just documentation of the rule.
- **Wave 0's ~95s average latency was investigated and addressed in code, real hardware data, [ADR 0013](../.adr/0013-generation-model-1-5b-and-reduced-rag-top-k-for-latency.md)** (2026-07-26): diagnosed via `llama-generate`'s own timing logs, not guessed — the dominant cost is **prompt processing** of the RAG context (~2,500-3,600 tokens at ~30 tok/s = 60-120s), not generation (~15-35 tokens at ~6 tok/s = 2.5-6s). `docker stats` during a live request showed 1009% CPU — already using essentially all 8 Docker-allocated cores, confirming a genuine hardware ceiling (2016-era Intel i7-7820HQ, no GPU), not an under-threaded config. The project owner asked for <5s; real live A/B benchmarking of Qwen2.5-3B/1.5B/0.5B on this exact hardware, using real KB content as context, found **<5s is not reachable at any tested size without an unacceptable quality regression** (0.5B: missed a plainly-present date, once claimed to *be* Gaspare Spontini rather than an assistant referencing him; even 1.5B's near-zero-context floor is 7.5s). Settled on Qwen2.5-3B → **Qwen2.5-1.5B** + `RAG_TOP_K` 5 → **2**: real, measured ~2-3x improvement (~95s → ~30-45s), quality held up in testing. Full comparison table and rationale in ADR 0013. **This changed the live system mid-campaign** — §1's architecture summary and §10.3's latency numbers are updated to flag the old Wave 0 numbers as describing a configuration that no longer exists; a fresh latency re-measurement against the new config is real follow-up work, not yet done. One more live spot-check after finalizing the config: "Dove è nato Gaspare Spontini?" (where was he born) answered "Gaspare Spontini è nato il 14 novembre 1774" (his birth *date*, not the place) — factually correct content, but doesn't address what was actually asked. Consistent with the "some reliability regression on marginal/awkward phrasings" consequence already flagged in ADR 0013 — real evidence this needs watching in Wave 1 scoring, not assumed fine.
- **`RAG_MIN_SCORE` cannot be cleanly tuned to separate relevant from irrelevant content for this KB — a real, substantial, unresolved finding** (2026-07-26): after the model/top_k change above, the Category F fallback problem was still present, so `RAG_MIN_SCORE` was tuned empirically for real, testing 7 values against a fixed probe set (one out-of-KB weather question, one out-of-KB future-mayor question, two genuinely in-KB questions with verified answers):

  | `RAG_MIN_SCORE` | Weather (should refuse) | Gaspare death date (should answer) | Name-year 1939 (should answer) | Sindaco futuro (should refuse) |
  |---|---|---|---|---|
  | 0.35 (Wave 0 default) | ✗ answers | ✓ answers | — | ✗ answers (Q19 hallucination) |
  | 0.6 | ✗ answers | ✓ answers | not tested | not tested |
  | 0.75 | ✗ answers | ✓ answers | not tested | not tested |
  | 0.78 | ✗ answers | ✓ answers | ✗ wrongly refuses | ✓ refuses |
  | 0.82 | ✗ answers (1 source) | ✗ wrongly refuses | not tested | not tested |
  | 0.85 | ✓ refuses | ✗ wrongly refuses | ✗ wrongly refuses | ✓ refuses |
  | 0.9 | ✓ refuses | ✗ wrongly refuses | ✗ wrongly refuses | ✓ refuses |

  **No value in the tested range correctly handles all four probes.** Below ~0.8, out-of-KB questions still get answered; at ~0.82 and above, genuinely in-KB questions with verified, documented answers start getting wrongly refused — and the transition isn't even cleanly ordered (the weather question's best-matching chunk score is *not* reliably lower than the legitimate questions' best-matching chunk scores). This means relevance and irrelevance are not linearly separable by a single global cosine-similarity cutoff for this embedding model (`nomic-embed-text`) against this KB's actual content — likely because every document shares heavy vocabulary overlap around "Maiolati Spontini" regardless of topic, which can dominate a shared-vocabulary-sensitive embedding's similarity signal over real topical relevance. **Left at `RAG_MIN_SCORE=0.6`** — a modest improvement over the 0.35 default (still not a fix) chosen deliberately over a more aggressive value, because this session's testing showed aggressive values (0.85+) break basic, core, verified-fact answering entirely, which fails the project's Mission outright — worse than the hallucination risk being partially mitigated, not eliminated. **This needs a different technique, not further threshold tuning**: re-ranking retrieved chunks against the query, hybrid keyword+semantic search, or reconsidering the embedding model. Real follow-up work, not closed by this session — do not re-attempt pure threshold trial-and-error expecting a different outcome without changing the underlying technique.
- **Real regression evidence against ADR 0013's "quality held up" conclusion, found on a larger real sample** (2026-07-26, Wave 1 sub-batch, 29 questions — full per-question detail in `.project/test-reports/TEST-INGESTION-0001-rep.md`): ADR 0013's "quality held up in testing" claim for Qwen2.5-1.5B + `RAG_TOP_K=2` was based on 2-3 live spot-check questions. This session's real, larger sample (29 questions, categories A/B/C/F) found **6 confirmed hallucinations (20.7%)**, well above Wave 0's 1/20 (5%) on the old 3B/top_k=5 config, plus **3 confirmed refusals on facts genuinely present in the ingested text** (province, population — both verified in Appendix B's own Wikipedia opening-line quote; and the Fraticelli castle-destruction year, already correctly retrieved from `storia-comune.md` in Wave 0 Q10). Notably: one hallucination (Q3, "Sei un funzionario comunale?") had the bot claim to **be** Gaspare Spontini — exactly the failure ADR 0013's own benchmarking flagged as a **0.5B-only** risk, now reproduced on the **deployed 1.5B model** in real traffic. Two other confirmed hallucinations directly contradicted verified, ingested source text (region: answered "Emilia-Romagna" instead of "Marche"; comune autonomy year: answered "1939" instead of "1808") rather than being vague or uncertain — they were stated with full confidence against a source that says the opposite. Plausible root cause for the present-fact refusals: halving `RAG_TOP_K` (5→2) reduces the chance the one relevant chunk lands in the retrieved set. **Not fixed by this session** — this needs the same kind of live A/B investigation ADR 0013 itself used (e.g. try `RAG_TOP_K=3-4` while keeping the 1.5B model, to isolate whether top_k or model size is the dominant driver) before any further wave is run at scale. One partial positive: a new prediction-type Category F question (like Wave 0's mayor-hallucination Q19) was correctly refused this time, suggesting ADR 0012's explicit no-predictions rule is helping at the content level even though the structural fallback mechanism (`fell_back:true`/zero citations) still never fires.
